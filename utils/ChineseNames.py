...
class ChineseNames:
    def lookup(self,df, key, lookup_df, lookup_key, lookup_value):  # 用于查找数据
        if type(lookup_key) == list:
            ...
            merged.drop(columns=merged.columns[merged.columns.str.contains('_suffix',regex=False)].tolist(), inplace=True)
        else:
            df2 = lookup_df[[lookup_key, lookup_value]].copy()
    ...
    def compute_name_index(self,data=None, fullname=None, surname=None, givenname=None, birthyear=None,
                           ...
                           NU_approx=True, digits=2, return_namechar=True, return_all=False):
        '''
    ...
    （√）CCU: Character corpus uniqueness, 字符语料库独特性1~6(基于当代中文语料库中某个字符的使用频率来计算的独特性指标。与NU不同，CCU衡量的是日常语言使用中字符的流行度，而不是名字中的使用频率。)
        NG: Name gender, 名字性别  -1（完全女性化；100% 由女性使用）到 0（性别中立；一半由女性使用，一半由男性使用）到 1（完全男性化；100% 由男性使用）。
    ...
    （√）NC: Name competence, 名字能力/自信 基于10位中文评价者对名字中包含的字符可能带来的能力相关特质的主观评分（1到5分）。（1 =极不可能具备，3 =可能性中等，5 =极有可能具备）。
        ...
        familyname = pd.read_csv(params.data_file_os+'name_data/familyname.csv')
        givenname = pd.read_csv(params.data_file_os+'name_data/givenname.csv')
...
if __name__ == '__main__':
    print('test..')
    cn = ChineseNames()
    result = cn.compute_name_index(name=["赵彩数",'周珊琳','王新鹏'], birth=[2024,1996,1996])
    print(result)
