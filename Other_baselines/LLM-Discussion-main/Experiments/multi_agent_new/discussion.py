...
class LLM_Debate_Instance_Similarities_Baseline(LLM_Debate):
    def run(self):
        ...
        for example in dataset['Examples']:
            ...
            for round in range(self.rounds):
                ...
                for agent in self.agents:
                    ...
                    if is_first_round:
                        formatted_initial_prompt = agent.construct_user_message(
                            agent_role_prompt + "Can you answer the following question as creatively as possible: " + initial_prompt + " Please put your answer in a list format, starting with 1. ... 2. ... 3. ... and so on.")
        ...
        output_file = self.save_debate_conversations(self.agents, all_responses, init_results, final_results,
                                                     amount_of_data, task_type=self.task_type, baseline=True)
        return output_file
...
class LLM_Discussion_Instance_Similarities(LLM_Debate):
    def run(self):
        # with open(self.dataset_file, 'r') as f:
        #     dataset = json.load(f)
        f = '/home/zsl/audrey_code/AI_Naming/dataset/test_dataset/benchmark/test_data_500.csv'
        df = pd.read_csv(f)
        dataset = {'Examples': df['query'].tolist()}
        all_responses = {}
        init_results = []
        final_results = []
        amount_of_data = len(dataset['Examples'])
